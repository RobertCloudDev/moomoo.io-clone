...
(function (global) {
  ...
  /* ---------------------------------------------------------------- consts */
  /* Processing's REAL numeric constants. This matters more than it looks:
     programs index their own arrays with them -- clicked[LEFT], keys[LEFT],
     keys[UP] -- so LEFT has to be 37, and mouseButton has to be a number
     rather than a descriptive string or those lookups silently miss. */
  var CORNER = 0, CORNERS = 1, RADIUS = 2, CENTER = 3, DIAMETER = 3;
  var BASELINE = 0, TOP = 101, BOTTOM = 102;
  var LEFT = 37, RIGHT = 39, UP = 38, DOWN = 40;
  var CLOSE = 2;
  var SQUARE = 1, ROUND = 2, PROJECT = 4, MITER = 8, BEVEL = 32;
  var BLUR = 11, GRAY = 12, INVERT = 13, OPAQUE = 14, POSTERIZE = 15,
      THRESHOLD = 16, ERODE = 17, DILATE = 18;
  var JAVA2D = 1, P2D = 2, P3D = 3, OPENGL = 4;
  var ARROW = 0, CROSS = 1, HAND = 2, MOVE = 3, TEXT = 4, WAIT = 5;
  var PIE = 3, OPEN = 1, CHORD = 2;
  var BACKSPACE = 8, TAB = 9, ENTER = 10, RETURN = 13, ESC = 27, DELETE = 127,
      SHIFT = 16, CONTROL = 17, ALT = 18, CODED = 0xffff;

  var CURSOR_CSS = { 0: "default", 1: "crosshair", 2: "pointer",
                     3: "move", 4: "text", 5: "wait" };
  ...
  /* Processing's color() ends with a fallback nobody expects: when the first
     argument is null or undefined it returns the colour-mode maxima -- WHITE,
     not black. Programs lean on it by accident. Spell Bound's Wizard() takes a
     `haircolor` parameter but is called with six arguments, so every
     fill(haircolor) inside it is fill(undefined); on Khan Academy the wizard's
     hair, beard, eyebrows and eye whites come out white because of this rule.
     NaN is NOT covered: Processing treats a NaN as a number and lands on
     black, so leave that path alone. */
  function isNil(v) { return v === null || v === undefined; }

  function color(a, b, c, d) {
    if (a instanceof Object && a.__col) return a;
    if (!arguments.length || isNil(a)) return packRGBA(255, 255, 255, 255);
  ...
  function css(args) {
    var r, g, b, a = 255, v;
    /* Same maxima fallback as color(); see the note there. */
    if (!args.length || isNil(args[0])) return "rgba(255,255,255,1)";
    if (args.length === 1) {
      v = args[0];
  ...
  function text(str, x, y) {
    ...
    ctx.textAlign = st.textAlignX === CENTER ? "center"
                  : (st.textAlignX === RIGHT ? "right" : "left");
    ctx.fillStyle = st.fill;

    var lines = str.split("\n");
    var lead = st.leading || st.textSize * 1.2;
    ...
    if (st.textAlignY === CENTER) {
      ...
      /* BASELINE: the FIRST line sits on y and later lines go DOWN.
         Shifting the block up by its own height (which is what BOTTOM
         wants) made every multi-line string climb over whatever was
         drawn above it -- and leading blank lines, which programs use
         to push a description below a heading, pushed it up instead. */
  ...
  function cursor(kind) {
    if (kind === undefined) { P.__cursor = "default"; return; }
    P.__cursor = CURSOR_CSS[kind] || (typeof kind === "string" ? kind : "default");
  }
  ...
  var API = {
    ...
    DIAMETER: DIAMETER, CLOSE: CLOSE, LEFT: LEFT, RIGHT: RIGHT,
    UP: UP, DOWN: DOWN, P3D: P3D, OPENGL: OPENGL, CODED: CODED,
    POSTERIZE: POSTERIZE, THRESHOLD: THRESHOLD, ERODE: ERODE, DILATE: DILATE,
    ...
    textAlign: textAlign, textLeading: textLeading, text: text, textWidth: textWidth,
...
/* =========================================================================
 * Additions covering the rest of the Khan Academy surface.
 * ========================================================================= */
(function (global) {
  "use strict";
  var P = global.KAProcessing, API = P.API;

  /* ---------------------------------------------------------------- sound */
  /* KA's audio API:
   *     var s = getSound("rpg/hit-thud");
   *     s.audio.volume = 0.3;
   *     s.audio.preservesPitch = false;
   *     s.audio.playbackRate = 1.3;
   *     playSound(s);
   * Programs poke .audio directly, so the returned object has to expose a
   * real HTMLAudioElement-shaped thing whether or not any file loads.
   * Drop matching .mp3 files into sounds/ to hear them; with none present
   * everything stays silent and nothing throws. */
  P.soundBase = "sounds/";
  P.soundsEnabled = true;
  var soundCache = {};

  function silentAudio() {
    return { volume: 1, playbackRate: 1, preservesPitch: true, currentTime: 0,
             loop: false, muted: false, paused: true,
             play: function () { return Promise.resolve(); },
             pause: function () {}, load: function () {},
             cloneNode: function () { return silentAudio(); } };
  }

  function getSound(name) {
    if (soundCache[name]) return soundCache[name];
    var audio;
    if (P.soundsEnabled && typeof global.Audio === "function") {
      try {
        audio = new global.Audio();
        audio.src = P.soundBase + name + ".mp3";
        audio.preload = "auto";
        /* A missing file must not become a console full of errors. */
        audio.addEventListener("error", function () { P.__missingSounds[name] = true; });
      } catch (e) { audio = silentAudio(); }
    } else {
      audio = silentAudio();
    }
    var snd = { audio: audio, name: name, __kaSound: true };
    soundCache[name] = snd;
    return snd;
  }
  P.__missingSounds = {};
  ...
  function playSound(snd) {
    ...
    try {
      ...
      var r = snd.audio.play();
      if (r && r.catch) r.catch(function () {});
    } catch (e) { /* autoplay policy or missing file; stay silent */ }
  }
  function stopSound(snd) {
    if (!snd || !snd.audio) return;
    try { snd.audio.pause(); snd.audio.currentTime = 0; } catch (e) {}
  }

  /* --------------------------------------------------------- seeded random */
  /* randomSeed() has to make random() reproducible, which means random()
     can no longer just be Math.random. */
  var seeded = false, seedState = 0;
  function nextRandom() {
    if (!seeded) return Math.random();
    seedState = (seedState * 1664525 + 1013904223) >>> 0;
    return seedState / 4294967296;
  }
  function randomSeed(s) { seeded = true; seedState = (s >>> 0) || 1; }
  function random(a, b) {
    if (a === undefined) return nextRandom();
    if (b === undefined) { b = a; a = 0; }
    return a + nextRandom() * (b - a);
  }

  /* ------------------------------------------------------- array utilities */
  /* Processing's append() returns a NEW array; it does not mutate. Programs
     that call it bare -- append(arr, v) with no assignment -- are therefore
     no-ops, which is a real and load-bearing quirk in at least one game. */
  function append(arr, el) { var out = arr.slice(); out.push(el); return out; }
  function shorten(arr) { var out = arr.slice(); out.pop(); return out; }
  function expand(arr, n) { return arr.slice(0, n === undefined ? arr.length * 2 : n); }
  function concat(a, b) { return a.concat(b); }
  function subset(arr, off, len) {
    return len === undefined ? arr.slice(off) : arr.slice(off, off + len);
  }
  function reverse(arr) { return arr.slice().reverse(); }
  function sort(arr, n) {
    var out = arr.slice(0, n === undefined ? arr.length : n).sort(function (x, y) {
      return typeof x === "number" ? x - y : String(x).localeCompare(String(y));
    });
    return n === undefined ? out : out.concat(arr.slice(n));
  }

  /* ------------------------------------------------------------ 3D no-ops */
  /* `normal` exists in Processing as a 3D function. One game passes it to
     rectMode() by accident -- rectMode(normal) -- which is harmless there
     but a ReferenceError here unless the name exists. */
  function noop3d() {}

  /* ---------------------------------------------------------- conversions */
  function int(v) { return typeof v === "string" ? (parseInt(v, 10) || 0) : (v | 0); }
  function float(v) { var n = parseFloat(v); return isNaN(n) ? 0 : n; }
  function str(v) { return String(v); }
  function boolean(v) { return !!v; }
  function nf(n, left, right) {
    var s = right === undefined ? String(n) : Number(n).toFixed(right);
    var parts = s.split(".");
    while (left && parts[0].replace("-", "").length < left) {
      parts[0] = parts[0].replace(/^(-?)/, "$10");
    }
    return parts.join(".");
  }

  var EXTRA = {
    getSound: getSound, playSound: playSound, stopSound: stopSound,
    randomSeed: randomSeed, random: random,
    append: append, shorten: shorten, expand: expand, concat: concat,
    subset: subset, reverse: reverse, sort: sort,
    normal: noop3d, lights: noop3d, noLights: noop3d, camera: noop3d,
    ambientLight: noop3d, directionalLight: noop3d, pointLight: noop3d,
    box: noop3d, sphere: noop3d, translateZ: noop3d,
    tint: noop3d, noTint: noop3d, blend: noop3d, copy: noop3d,
    requestImage: function () { return null; },
    int: int, float: float, str: str, boolean: boolean, nf: nf,
    binary: function (v) { return (v >>> 0).toString(2); },
    hex: function (v) { return (v >>> 0).toString(16).toUpperCase(); },
    unhex: function (v) { return parseInt(v, 16); },
    trim: function (v) { return String(v).trim(); },
    split: function (v, d) { return String(v).split(d); },
    splitTokens: function (v, d) { return String(v).split(new RegExp("[" + (d || " \\t\\n\\r\\f") + "]+")); },
    join: function (a, d) { return a.join(d); },
    match: function (s2, r) { return String(s2).match(new RegExp(r)); },
    day: API.day, month: API.month, year: API.year
  };

  for (var k in EXTRA) { if (EXTRA.hasOwnProperty(k)) API[k] = EXTRA[k]; }

  /* rnd inside baked artwork must follow randomSeed too. */
  P.__random = random;
})(typeof window !== "undefined" ? window : globalThis);
