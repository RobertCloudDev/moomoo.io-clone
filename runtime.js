...
(function (global) {
  ...
  function installGlobals(target) {
    ...
    target.CODED = 0xffff;
    target.focused = true;
    target.mouseButton = 37;
    /* Some programs print this.__frameRate as a debug readout. */
    target.__frameRate = 60;
  ...
  function bindInput(el, target) {
    ...
    el.addEventListener("mousedown", function (e) {
      ...
      /* Numeric, because programs write clicked[mouseButton] and then test
         clicked[LEFT]. Processing: LEFT 37, RIGHT 39, CENTER 3. */
      target.mouseButton = e.button === 2 ? 39 : (e.button === 1 ? 3 : 37);
    ...
    /* Processing's key constants do not all line up with the browser's
       keyCodes. ENTER is 10 in Processing but 13 in the DOM, and DELETE is
       127 rather than 46. Programs written on Khan Academy test the
       Processing values -- MooMoo checks keys[10] to leave the shop and to
       pause -- so translate before handing the code to the sketch. */
    function kaKeyCode(e) {
      if (e.keyCode === 13) return 10;   /* ENTER  */
      if (e.keyCode === 46) return 127;  /* DELETE */
      return e.keyCode;
    }

    /* Enter also carries a character in Processing: "\n". Without this a
       program that appends `key` to a string on Enter appends "65535". */
    function kaKey(e) {
      if (e.keyCode === 13) return "\n";
      return isPrintable(e) ? e.key : target.CODED;
    }

    global.addEventListener("keydown", function (e) {
      target.keyIsPressed = true;
      target.keyCode = kaKeyCode(e);
      target.key = kaKey(e);
  ...
  function step() {
    ...
    /* Rolling FPS, exposed because a couple of programs draw __frameRate. */
    var now = (global.performance && global.performance.now)
      ? global.performance.now() : Date.now();
    if (P.__fpsLast) {
      var dt = now - P.__fpsLast;
      if (dt > 0) target.__frameRate = Math.round(
        target.__frameRate * 0.9 + (1000 / dt) * 0.1);
    }
    P.__fpsLast = now;
